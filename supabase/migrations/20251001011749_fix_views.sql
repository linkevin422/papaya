...
-- Recreate clean views (invoker rights by default)

CREATE VIEW public.v_edge_entry_twd AS
SELECT 
    id,
    edge_id,
    entry_date,
...
CREATE VIEW public.v_edge_recurring_flow AS
WITH base AS (
    SELECT 
        ee.edge_id,
...
CREATE VIEW public.v_flow_totals_twd AS
SELECT 
    e.flow_id,
...
CREATE VIEW public.v_flow_headline AS
SELECT 
    e.flow_id,
